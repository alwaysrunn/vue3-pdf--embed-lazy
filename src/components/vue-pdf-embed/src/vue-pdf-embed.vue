<template>
  <div :id="id" class="vue-pdf-embed" ref="vuePdfRef">
    <div v-for="pageNum in pageNums" :key="pageNum" :id="id && `${id}-${pageNum}`" ref="elementRefs">
      <canvas />
...
//尤其重要，不加会导致线上不加载pdf 
pdf.GlobalWorkerOptions.workerSrc = 'pdfjs-dist-res/pdf.worker.js';
  ...
  try {
    ...
    await Promise.all(
      pageNums.map(async (pageNum, i) => {
        ...
        iframe.contentWindow.doc.body.appendChild(canvasClone)
...
async function renderStep(v: number) {
  let direction = pageCache.value > v ? 'top' : 'bottom';
  let result = Array(props.step).fill(v).map((it, idx) => direction === 'bottom' ? (it + idx) : (it - idx)).filter(it => it <= pageCount.value && it >= 1);
  pageCache.value = v;
  try {

    await Promise.all(
      result.map(async (pageNum, i) => {
        const page = await doc.value.getPage(pageNum)
        const pageRotation = props.rotation + page.rotate
